<?php
    ...
    $var1 = $_GET["kodeKelas"];
    $kelas = query("SELECT * FROM kelas WHERE kodeKelas = '$var1'")[0];
...
<html lang="en">
    ...
    <form action = "" method = "post">
        ...
        <div class="row mb-3">
            ...
            <input type="text" class="form-control" id="namaKls" name = "namaKls" value="<?= $kelas["nama_kelas"]; ?>">
            
        </div>

        <div class="row mb-3">
            <label for="nim" class="form-label">Fakultas</label>
            <input type="text" class="form-control" id="fakultas" name = "fakultas" value="<?= $kelas["fakultas"]; ?>">
            ...
            <div class="col-sm-10">
                ...
                <?php 
                    ...
                    while($Datadosen = mysqli_fetch_assoc($Search)) : 
                        if($Datadosen["id_dosen"] == $kelas["id_dosen"]) {
                            print '<option value="'.$kelas["id_dosen"].'" class="text-primary" selected>'.$Datadosen["nama_dsn"].'</option>';
                        } else {
                            print '<option value="'.$Datadosen["id_dosen"].'">'.$Datadosen["nama_dsn"].'</option>';
                        }
                ?>
                <?php endwhile ?>
                </select>
